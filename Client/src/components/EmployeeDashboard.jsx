...
const EmployeeDashboard = ({ data }) => {
  ...
  const cards = [
    ...
    {
      icon: DollarSignIcon,
      value: data.LatesPayslip
        ? `$${data.LatesPayslip.netSalary?.toLocaleString()}`
        : "N/A",
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8">
        {cards.map((card, index) => (
          <div
            key={index}
            className="card card-hover p-5 ms:p-6 relative overflow-hidden group flex items-center justify-between"
          >
            <div>
